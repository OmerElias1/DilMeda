...
function ObstacleView({ obs }: { obs: Obs }) {
  ...
  return (
    <View style={{
      position: 'absolute', left: cx - bw / 2, top: sy - bh, width: bw, height: bh,
      ...
      <View style={[s.game, flash && s.gflash]} {...pan.panHandlers}>
        ...
        <View style={s.sky}>
          ...
          {BLDGS.map((b, i) => {
            ...
            return (
              <View key={i} style={{
                position: 'absolute', bottom: 0, left, width: b.w, height: b.h,
                ...
                {[...Array(Math.floor(b.h / 22))].map((_, r) => [...Array(Math.floor(b.w / 15))].map((_, c) => (
                  <View key={`${r}${c}`} style={{
                    position: 'absolute', top: 5 + r * 20, left: 3 + c * 13,
        ...
        {/* Player */}
        <A.View style={{
          position: 'absolute', top: PLAYER_Y - 31,
          transform: [{ translateX: playerX }, { translateX: -(34 / 2) }, { translateY: jumpY }]
        }}>
          <View style={{
            position: 'absolute', width: 40, height: 68, borderRadius: 20,
            backgroundColor: 'rgba(255,107,53,0.15)', top: -3, left: -3
          }} />
          <Runner runAnim={runAnim} />
          <View style={{
            width: 30, height: 6, borderRadius: 15, backgroundColor: 'rgba(0,0,0,0.35)',
